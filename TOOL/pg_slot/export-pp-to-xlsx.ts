...
async function exportPGToXLSX() {
    try {
        // 读取 pg.yml 文件
        const yamlPath = path.join(__dirname, 'assets', 'pg.yml');
        ...
        const excelData: any[] = sort.map((symbol, index) => {
            ...
            try {
                const yamlPath = path.join(process.cwd(), 'assets', 'pg', symbol, 'payout-table.yml');
                let mul: any = {
                    default: {},
                    freegame: {}
                };
                ...
                normalBets = JSON.stringify(Object.keys(mul.default).sort((a, b) => parseFloat(a) - parseFloat(b)))
                freeBets = JSON.stringify(Object.keys(mul.freegame).sort((a, b) => parseFloat(a) - parseFloat(b)))
                
                normalBetsByNum = JSON.stringify(all, Object.keys(mul.default).sort((a, b) => parseFloat(a) - parseFloat(b)));
                freeBetsByNum = replaceAry(JSON.stringify(Object.keys(mul.freegame).sort((a, b) => parseFloat(a) - parseFloat(b))))
                let a = 1
