...
export class GameCollector {
    ...
    //免费游戏细节分布
    static detailFreePayoutObj: { [k: string]: any[] } = {
   ...
   private findDetailBucket(mul: number) {
    ...
    private freeDetailBucket(mul: number) {
        ...
        return this.findDetailBucket(mul)
    }
     ...
     private printFreeDetailBucket() {
        ...
        for (let key in GLOBAL_FREE_LIMIT) {
            printStr += key
            printStr += ":"
            printStr += GameCollector.detailFreePayoutObj[key]?GameCollector.detailFreePayoutObj[key].length.toString():0
            printStr += "/"
    ...
    private async sendCreateGame() {
        ...
        let appid = "faketrans2";
        let appsecret = "c21967e5-bd33-4a73-acbc-00da92a9cd92";
        let headers = {
        ...
        const resp1 = await fetch("https://hgapi365.com/api/v1/player/create", {
            method: "POST",
            headers: headers,
            body: JSON.stringify({
                UserID: UserID
        ...
        const resp2 = await fetch("https://hgapi365.com/api/v1/game/launch", {
            method: "POST",
            headers: headers,
            body: JSON.stringify({
                "UserID": UserID,
        ...
        const resp3 = await fetch("https://hgapi365.com/api/SetDemoPlayerRTP", {
            method: "POST",
            headers: headers,
            body: JSON.stringify({
                "DemoUserId": UserID,
            ...
            if (loadedData) {
                ...
                Object.keys(loadedData).forEach((key) => {
                    ...
                    for (let item of this.payoutObj["freegame"]) {
                        ...
                        if (bucket) {
                            if (!GameCollector.detailFreePayoutObj[bucket]) {
                                GameCollector.detailFreePayoutObj[bucket] = []
                            }
                            GameCollector.detailFreePayoutObj[bucket].push(mul)
                        }
                    }
                    let a = 1
                });
    ...
    async sendGameInfo() {
        ...
        let url = `https://api.hgapi365.com/game-api/${this.gameDirId}/v2/GameInfo/Get?traceId=${traceId}`
        ...
        // 若首次初始化文件 doInit.txt 不存在则写出一次
        if (this.gameDir&&false) {
            let initFile = "gameInfo";
            if(gameInfoData.dt.ls.si.fs&&gameInfoData.dt.ls.si.fs.ts){
                initFile  = "freeGameInfo"
            }
    ...
    async sendGameRule() {
        ...
        let url = `https://api.hgapi365.com/web-api/game-proxy/v2/GameRule/Get?traceId=${traceId}`
        const resp = await fetch(url, {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
    ...
    async initSession() {
        ...
        let traceId = createTraceId_27()
        let url = `https://api.hgapi365.com/web-api/auth/session/v2/verifySession?traceId=${traceId}`
        const resp = await fetch(url, {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
        ...
        this.baseUrl = "https://api.hgapi365.com/" + sessionData.dt.geu + "v2/spin"
    ...
    async runLoop(maxSamples = 1000) {
        ...
        while (true) {
            ...
            if (isCollect) {
                ...
                const bet = this.cs * this.maxLine* this.ml//this.toNum(tables[0].dt.si.tbb)
                ...
                /* let lowBets =[{"0.25":8},{"0.75":9},{"0.55":4},{"0.95":5},{"0.65":6},{"0.85":3},{"0.35":3}]
                ...
                if (this.free) {
                    ...
                    if (detailBucket) {
                        ...
                        if (!GameCollector.detailFreePayoutObj[detailBucket]) {
                            GameCollector.detailFreePayoutObj[detailBucket] = []
                        }
                        if (!(GameCollector.detailFreePayoutObj[detailBucket].length < (GLOBAL_FREE_LIMIT[detailBucket]))) {
                            this.resetRound(tables)
                           continue
                        }
 
                        GameCollector.detailFreePayoutObj[detailBucket].push(mul)
                ...
                this.payoutObj[bucket].push({
                    ...
                    mul: mul.toFixed(2),
                    startSc:0,
                    freeInFree:this.checkFreeInFree(tables),
                    check: false,
                });
