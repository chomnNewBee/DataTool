...
export class GameCollector {
   ...
   private findDetailBucket(mul: number) {
    ...
    async sendGameInfo() {
        ...
        url = `https://api.icgxzz3el.com/${this.geu}v2/GameInfo/Get?traceId=${traceId}`
    ...
    async sendGameRule() {
        ...
        url = `https://api.icgxzz3el.com/web-api/game-proxy/v2/GameRule/Get?traceId=${traceId}`
        const resp = await fetch(url, {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
    ...
    async initSession() {
        ...
        let url = `https://api.icgxzz3el.com/web-api/auth/session/v2/verifySession?traceId=${traceId}`
        const resp = await fetch(url, {
            method: "POST",
            headers: {
                "Content-Type": "application/x-www-form-urlencoded"
        ...
        this.baseUrl = "https://api.icgxzz3el.com/" + sessionData.dt.geu + "v2/Spin"
        this.geu = sessionData.dt.geu
        this.token = sessionData.dt.tk
    ...
    async runLoop(maxSamples = 1000) {
        ...
        while (true) {
            ...
            if (obj.dt.si.tbb > 0) {
                obj.dt.si.tbb = this.cs * this.maxLine * this.ml
            }
            if(obj.dt.si.tb>0){
                obj.dt.si.tb=this.cs * this.maxLine * this.ml
            }
            ...
            if (isCollect) {
                ...
                let lowBets =[{"1":9},{"0.7":6},{"0.8":4},{"0.9":6}];
                if (!this.free)
                    for (let items of lowBets) {
                        const [key, value] = Object.entries(items)[0];

                        if (GameCollector.extra_data[key] == undefined) {
                            GameCollector.extra_data[key] = value
                        }
                        if (GameCollector.extra_data[key] < 10 && Math.abs(Number(key) - mul) < 0.0001) {
                            GameCollector.extra_data[key]++
                ...
                if ( mul > 270 ||(!this.free && mul >100)|| (bucket == "over_30")&&this.payoutObj[bucket].length >70)  {
                    this.resetRound(tables)
                    continue
                }
